# Chapter 13: Merging Local and Remote Tools

By the end of this chapter the system is finished: your agent discovers the weather server's tools at startup, merges them with the local ones into a single list, and dispatches each call to the right place, a dict lookup or a wire protocol, without the model knowing there's a difference. This is the book's last construction chapter, and it closes with the complete final file.

## The One Real Wrinkle: Async, and Connection Lifetime

The MCP client SDK is async, and the rest of your file is synchronous. That collision forces one design decision, and it's worth making deliberately because the lazy version works just well enough to ship and just badly enough to embarrass you.

The lazy version: leave `run_agent` synchronous and wrap each MCP call in `asyncio.run(...)` at the dispatch site. It runs. It also tears down the entire client connection after every call, and for a stdio server, the connection *owns the subprocess*. Every single `get_weather` call would spawn a fresh Python process, speak the protocol handshake, make one call, and kill the process. The trace would look fine. The process table would tell the truth.

The right shape is one you've already built once: `_kb_collection` is a lazily-initialized, process-lifetime handle precisely so Chroma isn't reconnected on every search. Same instinct here, made structural: **open the MCP connection once, for the life of the run, and thread it through.** Concretely, `run_agent` becomes `async def`, and `main` opens the connection in a single `async with` that everything else runs inside.

What about the OpenAI SDK calls inside the loop? They stay ordinary synchronous calls. Yes, they block the event loop for the duration of each model call; for a single-user local demo with nothing else scheduled, that's a non-issue, and converting them to an async client would add surface area for zero observable benefit here. A production service multiplexing many conversations would make the opposite call. Name the context, choose accordingly, write it down: by now that's reflex.

## New Imports and the Server Handle

```python
# WHERE: add to the existing import block at the top of agentic_demo.py
import asyncio
...
```python
# WHERE: insert at module level, directly after _check_prompt_injection_llm()
MCP_SERVER_PARAMS = StdioServerParameters(command="python3", args=["mcp_weather_server.py"])
```

`StdioServerParameters` is the launch recipe: the client spawns `python3 mcp_weather_server.py` as a subprocess and owns its stdin/stdout. No port, no registry, no discovery service; for local servers, the "address" is a command line.

## Discovery: Schemas From the Wire

```python
# WHERE: insert at module level, directly after MCP_SERVER_PARAMS
...
Read what the translation amounts to: relabeling. `list_tools()` hands back name, description, and `input_schema`, and the schema is *already JSON Schema*, the same dialect you hand-wrote in Chapter 3. Wrapping it in the `{"type": "function", ...}` envelope is the entire adaptation between "MCP server's self-description" and "what the OpenAI-compatible API expects." That near-zero translation cost isn't luck; both sides standardized on JSON Schema because everyone did, and it's a large part of why the MCP ecosystem could form at all.

The returned `names` set matters as much as the schemas. The model's `tool_calls` response carries only a tool *name*, with no indication of where the tool lives, so the router needs its own memory of which names arrived via discovery. That's this set.

## Invocation: Normalizing What Comes Back

Chapter 12 ended with a promise: the client would handle the result contract as it actually is, not as the docs imply. Here's that code.

```python
# WHERE: insert at module level, directly after _discover_mcp_tools()
...
Three cases, one contract. Errors don't raise across the wire; `is_error` is the signal and the message rides in a text block, which this function converts into the same `{"error": ...}` dict your local tools have returned since `calculate` in Chapter 3. Structured content is used when the server provides it. And the fallback parses the text block back into a real dict, closing Chapter 12's gotcha, with a final safety net wrapping unparseable text as `{"result": text}`. The function's whole job is expressed in its return type: whatever happens on the wire, the loop sees the same plain dict it has always seen. Normalization at the boundary is what keeps the boundary invisible.

## The Loop: Four Small Changes

`run_agent` changes in exactly four places, and none of them is control flow.

The signature goes async and accepts the connection:

```python
# WHERE: replace run_agent's `def` line
async def run_agent(
    user_prompt: str,
...
The tool list the model sees becomes the merge, replacing Chapter 8's line:

```python
# WHERE: in run_agent, replace the line
#   tools_for_this_turn = None if force_text_only else TOOL_SCHEMAS
        ...
        # mcp_client was passed in) — the model sees one merged tool list
        # either way, with no indication in the schema itself of which tools
        # are local vs. MCP-sourced. That's deliberate: the model shouldn't
        # need to know or care where a tool lives, only that it exists and
        # what it does.
        tools_for_this_turn = None if force_text_only else TOOL_SCHEMAS + (mcp_tool_schemas or [])
```

Dispatch becomes a router, replacing the two-line lookup at the seam:

```python
# WHERE: in run_agent's tool-dispatch loop, replace the two lines
...
And the verbose print gains its provenance label, so the trace shows you what the model can't see:
...
That's the whole conversion. The guardrails around these lines, the backstop's `force_text_only`, the citation tracker, the compaction call, are character-for-character what they were in Chapter 9. Chapter 11 made a claim about the seam; this is the claim, kept.

## Startup: One Connection, Everything Inside It

`main` becomes async and owns the connection:

```python
# WHERE: replace the entire `if __name__ == "__main__":` block with
# this async main() and the asyncio.run entry point
async def main() -> None:
    # Ingest and calibrate before the agent loop, so their output doesn't
...
The `async with` block is the connection's lifetime: subprocess spawned and handshake spoken on entry, subprocess torn down on exit, and every agent run threads through the one live client. Note also that `run_agent`'s MCP parameters all default to `None`: a caller that doesn't care about weather can still invoke the agent with no MCP server at all, and the merge line's `(mcp_tool_schemas or [])` quietly degrades to local-only. Optional integration, not a new requirement.
...
The startup sequence now reads: ingest (or the skip line), the calibration table, then the new line, `[mcp] connected to mcp_weather_server.py — discovered tool(s): ['get_weather']`. A tool your agent has never seen a schema for is now in the model's list.

The first prompt proves the wire works: `executing tool (mcp)`, a subprocess round-trip, and Austin's fake 101°F rendered for a five-year-old.

The second prompt is the one to savor, because it's the whole book in a single turn. "Is Austin more than 3 times as warm as Seattle?" drives the model through `get_weather` twice, dispatched `(mcp)`, across a process boundary, then `calculate` for 3 × 68, dispatched `(local)`, then `compare` for 101 against 204, `(local)` again, because Chapter 3 taught it never to eyeball a magnitude judgment. Three tools, two sourcing mechanisms, one merged list, and the model treated them identically because from where it sits there's no difference to see. Meanwhile the handbook prompts run with every Part 2 guardrail intact, and the two probe prompts still get blocked and cleared exactly as before.

MCP is now genuinely wired into the loop: tools flowing from a local dict and from a real separate process over the actual wire protocol, indistinguishable to the model and to the rest of the agent loop. That sentence is what you built.

## The Code Changed This Chapter

Eight edits, in order, then the final file below to verify against.

**Edit 1:** add the four new imports (`asyncio`, and the three `mcp` imports) to the import block.
**Edit 2:** insert `MCP_SERVER_PARAMS` at module level, after `_check_prompt_injection_llm()`.
**Edit 3:** insert `_discover_mcp_tools()` directly after `MCP_SERVER_PARAMS`.
**Edit 4:** insert `_call_mcp_tool()` directly after `_discover_mcp_tools()`.
**Edit 5:** replace `run_agent`'s `def` line with the async signature.
**Edit 6:** in `run_agent`, replace the `tools_for_this_turn` line with the merged version, and replace the two-line local dispatch with the router (plus the `(dispatch_via)` label in the verbose print).
**Edit 7:** replace the `if __name__ == "__main__":` block with `async def main()` and the `asyncio.run(main())` entry point.
**Edit 8:** nothing in `mcp_weather_server.py`; it's unchanged from Chapter 12.

## The Complete File: agentic_demo.py, Final (Chapter 13 Checkpoint)

The finished system. Together with Chapter 12's `mcp_weather_server.py`, this is everything.
...
The system is complete. Part 4 closes the book: the lessons compressed into a reference you'll return to, and the road from this laptop demo to everything it was preparing you for.
